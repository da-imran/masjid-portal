...
const NotaKuliahPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchDownloads = async () => {
            try {
                const response = await api.get<DownloadItem[]>('/api/v1/downloads/nota_kuliah');
                // Ensure response.data is an array
                const data = Array.isArray(response.data) ? response.data : [];
                setDownloadList(data);
            } catch (err) {
                console.error('Failed to fetch nota kuliah:', err);
                setDownloadList([]);
            } finally {
                setLoading(false);
            }
    ...
    return (
        <div className="row">
            <div className="col-md-10">
                <h6 className="text mt-3" style={{ fontSize: '20px' }}><b>Nota Kuliah</b></h6>
                <div className="mt-5 mb-5 p-3 border">
                    {loading ? (
                        <p>Memuat nota kuliah...</p>
                        ...
                        <p>Tiada nota kuliah buat masa ini.</p>
                    )}
                </div>
            </div>
        </div>
